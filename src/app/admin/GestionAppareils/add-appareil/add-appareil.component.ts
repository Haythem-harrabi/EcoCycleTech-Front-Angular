...
export class AddAppareilComponent implements OnInit {
  ...
  onPredictPrix(): void {
    ...
    if (description && description.trim()) {
      console.log('Description:', description); // Afficher la description
      this.appareilService.predictPrix(description).subscribe({
        next: (response: any) => {
          // Assuming the response is { prix_pred: 782.59 }
          this.prixPrevu = Number(response.prix_pred); // Convert the value to a number
          console.log('Prix prédit:', this.prixPrevu); // Log the predicted price
        },
