...
minDate: any;
  ...
  private loadUsers(): void {
    this.reservationService.getAllUsers().subscribe({
      next: (users: User[]) => {
        this.allUsers = users;
      },
      error: (err:any ) => {
  ...
  private loadReservationForEdit(reservationId: number): void {
    ...
    // First ensure all appareils are loaded
      ...
      next: (appareils) => {
        ...
        this.reservationService.getReservationById(reservationId).subscribe({
          next: (reservation: Reservation) => {
            ...
            // Safe user handling
            let user = null;
            if (reservation.user && reservation.user.idUser) {
              user = this.allUsers.find(u => u.idUser === reservation.user?.idUser) || reservation.user;
            }
            ...
            // Clear and repopulate appareils
...
onAppareilChange(event: any, index: number): void {
  ...
  onSubmit(): void {
    ...
    // Vérifier que la liste des appareils est correctement intégrée dans les données
    const appareils = this.appareilsArray.controls.map(control => control.value);
    reservationData.panier = appareils;

    // Si en mode édition, mettre à jour la réservation, sinon créer une nouvelle
    this.isLoading = true;
    if (this.isEditMode && this.currentReservationId) {
      ...
      this.reservationService.createReservation(reservationData)
        .subscribe({
          next: () => {
            this.panierService.clearPanier(); // Nettoyer le panier après une nouvelle réservation
            this.router.navigate(['Reservations']);
          },
          error: (err) => {
            this.isLoading = false;
