...
export class AppareilService {
  ...
  getAppareils(): Observable<Appareil[]> {
    return this.http.get<Appareil[]>('/api/appareils')
      .pipe(
        catchError((error) => {
...
getAverageRating(appareilId: number): Observable<number> {
  return this.http.get<number>(`/api/avis/average/${appareilId}`);
}
  predictPrice(categorie: string, marque: string, averageRating: number): Observable<any> {
    console.log("IN PREDICTION " +categorie + marque + averageRating )
    return this.http.post(`http://localhost:5000/predict/predict-price`, {
      categorie,
      marque,
      averageRating
    });
  ...
  predictPrix(description: string): Observable<number> {
    return this.http.post<number>('http://localhost:5000/predict/predict-price', { description });
  }
