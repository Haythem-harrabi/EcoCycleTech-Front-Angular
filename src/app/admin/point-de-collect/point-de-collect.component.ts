...
// point Leaflet at your assets folder
...
export class PointDeCollectComponent implements OnInit, AfterViewInit {
  ...
  private initMap(): void {
    // if the <div id="map"> isn't in the DOM, bail out
    const container = document.getElementById('map');
    if (!container) {
      return;
    }
  
    // if we already have a map instance, tear it down
    if (this.map) {
      this.map.remove();
      this.map = null;
    }
    ...
    // now create it fresh
    this.map = L.map(container).setView([36.8065, 10.1815], 13);
  ...
  openEditModal(point: PointCollecte): void {
    ...
    setTimeout(() => {
      ...
      if (this.point.latitude && this.point.longitude) {
        ...
        // add or move marker
        if (this.marker) {
          this.marker.setLatLng([this.point.latitude, this.point.longitude]);
        } else {
          this.marker = L.marker([this.point.latitude, this.point.longitude]).addTo(this.map!);
        }
      }
    }, 200);
  }
