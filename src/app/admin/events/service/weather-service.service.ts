...
export class WeatherService {
  private API_KEY = '9d53f57e2eae380cf08e37500c99d1f2';
  ...
  getWeather(location: string): Observable<{temp: number; icon: string}|null> {
    ...
    return this.http.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(location)}&units=metric&appid=${this.API_KEY}`
    ).pipe(
      map((res: any) => ({
