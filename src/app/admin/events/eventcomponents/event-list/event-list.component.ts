...
isRegistrationOpen(event: Evenement): boolean {
  ...
  async registerForEvent(event: Evenement) {
    ...
    try {
      ...
      // 2. Prepare ticket data with QR code
      const ticketData = {
        evenement: { idEvenement: event.idEvenement },
        user: { idUser: 1 }, // Hardcoded user ID
        dateAchat: new Date(), // Current datetime
        qrCodeData: this.generateTicketHash(event.idEvenement, 1) // Generate unique hash
      };
  
      // 3. Create ticket with QR code
      const createdTicket = await lastValueFrom(
        this.ticketService.createTicketWithQR(ticketData as unknown as TicketEvenement)
      );
  
      // 4. Show success with ticket details
